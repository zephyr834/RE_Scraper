...
if(_satellite.settings.isStaging) s.account="homerealtordev";
else s.account="homerealtor";

// timeparting config variables
s.dstStart="03/13/2016"; // update to the correct Daylight Savings Time start date for the current year.
...
s.prop27 = window.location.hostname;

s.prop30=s_getLoadTime();
s.eVar30 = "D=c30";   
// Time without Hours, Minutes and Seconds

var f=new Date();s.eVar66=f.getFullYear()+"-"+(f.getMonth()+1)+"-"+f.getDate()

// Visitor ID Collection
s.eVar75 = "D=mid";
...
s.eVar1 = s.Util.getQueryParam("iid").toLowerCase();
  
s.prop74=s.getTimeParting('h','-8'); // Set hour 
...
/* Load Time */
function s_getLoadTime(){if(!window.s_loadT){var b=new Date().getTime(),o=window.performance?performance.timing:0,a=o?o.requestStart:window.inHeadTS||0;s_loadT=a?((b-a)/1000):''}return s_loadT};
...
*/
s.socialAuthors=new Function("a",""
...
s.maxDelay='750'; //max time to wait for 3rd party api response in milliseconds
...
/* THE CONTENTS OF THIS FILE SHOULD BE REPLACED WITH YOUR ADOBE-PROVIDED MEASUREMENT LIBRARY */
